...
The application source lives in the [`family-tree/`](family-tree/) directory.
...
- `family-tree/src/App.tsx` composes the graph, filters, profile sidebar, and onboarding tour.
- `family-tree/src/data/familyTree.ts` defines the initial people, nodes, edges, and helper lookup.
- `family-tree/src/components/FamilyNode.tsx` renders each individual or couple and applies filter highlighting.
- `family-tree/src/components/ProfileSidebar.tsx` displays and edits the selected person's local profile.
- `family-tree/src/components/FilterPanel.tsx` manages name and profile-completeness filters.
- `family-tree/src/context/ProfileContext.tsx` stores profile state and persists it to `localStorage`.
- `family-tree/src/context/FilterContext.tsx` provides filter state to the interface.
- `family-tree/src/utils/layout.ts` runs Dagre and maps its coordinates into React Flow positions.
...
No environment variable is required by the current application behavior. Although `family-tree/vite.config.ts` maps `GEMINI_API_KEY` and the dependency list includes `@google/genai`, the checked-in application source does not make a Gemini API request.
...
There is currently no automated test command in `family-tree/package.json`.
...
`family-tree/src/data/familyTree.ts` contains the people displayed in the graph and the node-to-node relationships used by React Flow. A person has an identifier, display name, optional role, optional gender marker, and an optional flag for the central member.
...
1. Open `family-tree/src/data/familyTree.ts`.
...
The layout utility currently uses fixed node dimensions and a top-to-bottom direction. Spacing and direction can be adjusted in `family-tree/src/utils/layout.ts`.
...
Review node and edge definitions in `family-tree/src/data/familyTree.ts`, then adjust `nodeWidth`, `nodeHeight`, `nodesep`, or `ranksep` in `family-tree/src/utils/layout.ts`.
